...
def upload_folder(credentials_path, bucket_name, destination_blob_prefix, local_dir):
    ...
    for root, _, files in os.walk(local_dir):
        for file in files:
            ...
            print(
                f"Uploaded {local_file_path} to gs://{bucket_name}/{remote_path}")
...
def upload_file(credentials_path, bucket_name, destination_blob_prefix, filepath):
    ...
    blob_address = os.path.join(
        destination_blob_prefix, os.path.basename(filepath))
    blob = bucket.get_blob(blob_address)
    if blob == None:
        blob = bucket.blob(blob_address)
    blob.upload_from_filename(
        "Semantics-Research/Upload-Test/test.txt", client=storage_client)
...
if __name__ == "__main__":
    ...
    upload_folder(credentials_path=service_account_path, bucket_name=bucket_name,
                  destination_blob_prefix=destination_blob_prefix, local_dir=local_dir)
    ...
    dataset = gcs_get_dataset(credentials_path=service_account_path,
                              bucket_name="project3102-data-bucket", data_blob_path="sample_1950s_1960s.jsonl")
